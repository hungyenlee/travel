...
- **全站側邊欄**：左側常駐導覽（探索城市／我的行程）。桌機把滑鼠移到「我的行程」會浮出行程清單快速跳轉；點「我的行程」進入總覽（列出所有行程、可建立新的）。手機版側邊欄收合為漢堡選單。
- **首頁**：城市入口 + 台南隨機精選景點與店家。
- **城市頁**：互動地圖（Leaflet + OpenStreetMap）＋篩選＋卡片列表，三者連動。景點藍點、美食橘點。
- **詳細頁**：景點／店家共用版型，含地址、營業時間、Google 地圖連結。
- **我的行程（行程規劃）**：把想去的地點加進命名行程、分配到「第 N 天」。規劃層一頁列出各天（分天區塊），卡片可用左側握把**拖曳排序**（同天重排、跨天搬移；手機長按啟動），地圖跟著選中的天顯示編號標記並與清單雙向連動。資料只存在你的瀏覽器（localStorage），不需帳號、不上傳。
...
│   └── plan.js         我的行程頁邏輯（地圖＋分天區塊清單＋拖曳排序）
...
- [`docs/adr/`](docs/adr/)：重大決策的來龍去脈（固定允許清單、地圖預覽卡自製、共用層架構、行程規劃的 localStorage 資料模型、全站側邊欄導覽、規劃層分天區塊與拖曳排序）。
